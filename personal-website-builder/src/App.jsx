// App.jsx
// ----------------------------------------------------------------------
// This is the main (and only) component for the page.
// It is kept in a single file on purpose so it is easy to read and explain.
// It shows:
//   1. A dark/light mode toggle (uses React state + localStorage)
//   2. A profile section (photo, name, bio)
//   3. A list of links rendered from an array using .map()
// ----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// LINKS DATA
// We store all the links in a simple array of objects.
// Keeping the data separate from the layout makes it easy to add/remove
// links later. Each link has a title, a url, a short description, and an
// "icon" name. The icon name is matched to a small SVG in getIcon() below.
// -----------------------------------------------------------------------
const links = [
  {
    title: "LinkedIn",
  ...
  {
    title: "Resume",
    url: "#", // Placeholder — replace with the real resume PDF link later.
  ...
  },
  {
    ...
    url: "#", // Placeholder — link to the project when it's ready.
...
// -----------------------------------------------------------------------
// "CURRENTLY" DATA
// A small list of what I'm up to right now. Stored as an array so it's
// easy to render with .map() and easy to edit later.
// -----------------------------------------------------------------------
const currently = [
...
// -----------------------------------------------------------------------
// ICONS
// A simple helper that returns a small SVG icon for a given name.
// Using a plain function (instead of a fancy component) keeps things
// beginner-friendly. "currentColor" makes each icon match the text color.
// -----------------------------------------------------------------------
...
function App() {
  ...
  // "isDark" is true when dark mode is on. We start by reading the saved
  // value from localStorage so the user's choice is remembered across visits.
  // The function inside useState only runs once, when the component loads.
  // ---------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------
  // APPLY THE THEME
  // This effect runs every time "isDark" changes.
  // It adds or removes the "dark" class on the <body> tag (our CSS uses
  // that class to switch colors), and saves the choice in localStorage.
  // ---------------------------------------------------------------------
  useEffect(() => {
    if (isDark) {
  ...
  return (
    ...
    <main className="page fade-in">
      {/* ----------------------------------------------------------------
          ...
          A couple of soft pixel feathers drifting in the background.
          They are purely decorative, so aria-hidden hides them from
          screen readers and pointer-events (set in CSS) keeps them from
          ever blocking clicks.
      ----------------------------------------------------------------- */}
      <img className="pixel-feather pixel-feather--one" src="/pixel-feather.png" alt="" aria-hidden="true" />
      <img className="pixel-feather pixel-feather--two" src="/pixel-feather.png" alt="" aria-hidden="true" />
      ...
      <section className="profile">
        <div className="profile-img-wrap">
          ...
          <img className="profile-img" src="/profile-placeholder.png" alt="Tavleen Kaur holding one of her birds" />
      ...
      {/* LINKS SECTION
          We loop over the "links" array with .map() and create one card per link.
      ...
      {/* CURRENTLY SECTION
          A small card that lists what I'm up to right now.
          We render the "currently" array with .map(). */}
